export default {
    settings: {
      ...
      imageGenerationQueue: {
        title: '生图任务队列保护',
        description: '限制服务器同时执行的生图任务数量，超出部分进入持久化等待队列，避免小规格服务器因并发生成而耗尽资源。',
        enabled: '启用生图队列保护',
        enabledHint: '关闭后不再使用服务器级生图并发限制；已有任务会继续处理。',
        maxActiveJobs: '最大同时执行任务数',
        maxActiveJobsHint: '只有处于上游提交、轮询、存储或结算生命周期内的任务会占用执行槽位。建议从 1 开始逐步调高。',
        maxQueuedJobs: '最大等待队列数',
      ...
      imageWorkbenchAnnouncements: {
        ...
        interval: '滚动周期（秒）',
        intervalHint: '每隔指定秒数滚动到下一条公告，范围 1–3600 秒。',
        add: '添加公告',
        empty: '暂未添加公告。',
      ...
      site: {
        ...
        contactPage: {
          title: '联系页面配置',
          description: '更新联系页展示的群号、频道链接和二维码，保存后立即生效，无需重新构建前端。',
          qqExpiryWarning: 'QQ群二维码通常存在有效期。二维码过期后只需在这里上传新图片；群号可长期保留作为备用加入方式。',
          qqTitle: 'QQ 用户交流群',
          qqGroupNumber: 'QQ群号',
          qqGroupNumberPlaceholder: '例如：1097280864',
          qqQrImage: 'QQ群二维码',
          telegramTitle: 'Telegram 频道',
          telegramName: '频道名称',
          telegramNamePlaceholder: "例如：{'@'}ai_baipiao",
          telegramUrl: '频道链接',
          telegramQrImage: 'Telegram 二维码',
          replaceImage: '上传或替换图片',
          imageHint: '支持 PNG、JPG、WEBP 或 GIF，最大 2MB。图片将保存在数据库设置中。',
        },
        docUrl: '文档链接',
      ...
      purchase: {
        title: '在线充值页面',
        description: '在侧边栏展示“在线充值”入口，并在页面内通过 iframe 打开指定链接',
        enabled: '显示在线充值入口',
