export default {
  ...
  userSubscriptions: {
    ...
    noPurchasedSubscriptionsDesc: '购买后，订阅卡会显示在这里，您可以手动启用。',
    failedToLoad: '加载订阅失败',
    ...
    activate: '立即启用',
    activateSuccess: '订阅已启用，有效期现在开始计算。',
    activateFailed: '启用订阅失败，请稍后重试。',
    readyToActivate: '待启用',
    validity: '有效期',
    planValidity: '{days} 天',
    status: {
  ...
  payment: {
    ...
    admin: {
      ...
      weeklyLimit: '周限额',
      monthlyLimit: '月限额',
      quotaOverride: '套餐限额覆盖',
      quotaOverrideHint: '填写大于 0 的值覆盖分组默认限额，填写 0 表示沿用分组配置。',
      unlimited: '无限制',
