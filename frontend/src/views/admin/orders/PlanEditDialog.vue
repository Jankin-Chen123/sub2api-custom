<template>
  <BaseDialog :show="show" :title="plan ? t('payment.admin.editPlan') : t('payment.admin.createPlan')" width="wide" @close="emit('close')">
    <form id="plan-form" @submit.prevent="handleSavePlan" class="space-y-4">
      ...
      <div>
        <label class="input-label">{{ t('payment.admin.quotaOverride') }}</label>
        <div class="grid grid-cols-3 gap-3">
          <div>
            <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.dailyLimit') }}</label>
            <input v-model.number="planForm.daily_limit_usd" type="number" min="0" step="0.01" class="input mt-1" />
          </div>
          <div>
            <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.weeklyLimit') }}</label>
            <input v-model.number="planForm.weekly_limit_usd" type="number" min="0" step="0.01" class="input mt-1" />
          </div>
          <div>
            <label class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.monthlyLimit') }}</label>
            <input v-model.number="planForm.monthly_limit_usd" type="number" min="0" step="0.01" class="input mt-1" />
          </div>
        </div>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.quotaOverrideHint') }}</p>
      </div>
      <div class="grid grid-cols-2 gap-4">
