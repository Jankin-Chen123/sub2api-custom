...
export async function mountChillLionWhenReady(element: HTMLElement): Promise<() => void> {
  await loadThree()
  return mountChillLion(element)
}
...
function init(){
  ...
  container.addEventListener('pointercancel', handlePointerUp, false);
  container.addEventListener('pointerleave', handlePointerUp, false);
  /*
...
function rule3(v,vmin,vmax,tmin, tmax){
  ...
  return () => {
    ...
    container.removeEventListener('pointermove', handlePointerMove, false)
    container.removeEventListener('pointerdown', handlePointerDown, false)
    container.removeEventListener('pointerup', handlePointerUp, false)
    container.removeEventListener('pointercancel', handlePointerUp, false)
    container.removeEventListener('pointerleave', handlePointerUp, false)
    if (renderer) {
