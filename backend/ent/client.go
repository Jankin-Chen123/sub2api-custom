...
type Client struct {
	...
	// ImageGenerationJob is the client for interacting with the ImageGenerationJob builders.
	ImageGenerationJob *ImageGenerationJobClient
...
func (c *Client) init() {
	...
	c.ImageGenerationJob = NewImageGenerationJobClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		ImageGenerationJob:            NewImageGenerationJobClient(cfg),
...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.IdentityAdoptionDecision, c.ImageGenerationJob, c.PaymentAuditLog,
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *ImageGenerationJobMutation:
		return c.ImageGenerationJob.mutate(ctx, m)
...
// ImageGenerationJobClient is a client for the ImageGenerationJob schema.
type ImageGenerationJobClient struct {
	config
}

// NewImageGenerationJobClient returns a client for the ImageGenerationJob from the given config.
func NewImageGenerationJobClient(c config) *ImageGenerationJobClient {
	return &ImageGenerationJobClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `imagegenerationjob.Hooks(f(g(h())))`.
func (c *ImageGenerationJobClient) Use(hooks ...Hook) {
	c.hooks.ImageGenerationJob = append(c.hooks.ImageGenerationJob, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `imagegenerationjob.Intercept(f(g(h())))`.
func (c *ImageGenerationJobClient) Intercept(interceptors ...Interceptor) {
	c.inters.ImageGenerationJob = append(c.inters.ImageGenerationJob, interceptors...)
}

// Create returns a builder for creating a ImageGenerationJob entity.
func (c *ImageGenerationJobClient) Create() *ImageGenerationJobCreate {
	mutation := newImageGenerationJobMutation(c.config, OpCreate)
	return &ImageGenerationJobCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of ImageGenerationJob entities.
func (c *ImageGenerationJobClient) CreateBulk(builders ...*ImageGenerationJobCreate) *ImageGenerationJobCreateBulk {
	return &ImageGenerationJobCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *ImageGenerationJobClient) MapCreateBulk(slice any, setFunc func(*ImageGenerationJobCreate, int)) *ImageGenerationJobCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &ImageGenerationJobCreateBulk{err: fmt.Errorf("calling to ImageGenerationJobClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*ImageGenerationJobCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for ImageGenerationJob.
func (c *ImageGenerationJobClient) Update() *ImageGenerationJobUpdate {
	mutation := newImageGenerationJobMutation(c.config, OpUpdate)
	return &ImageGenerationJobUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *ImageGenerationJobClient) UpdateOne(_m *ImageGenerationJob) *ImageGenerationJobUpdateOne {
	mutation := newImageGenerationJobMutation(c.config, OpUpdateOne, withImageGenerationJob(_m))
	return &ImageGenerationJobUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *ImageGenerationJobClient) UpdateOneID(id int64) *ImageGenerationJobUpdateOne {
	mutation := newImageGenerationJobMutation(c.config, OpUpdateOne, withImageGenerationJobID(id))
	return &ImageGenerationJobUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for ImageGenerationJob.
func (c *ImageGenerationJobClient) Delete() *ImageGenerationJobDelete {
	mutation := newImageGenerationJobMutation(c.config, OpDelete)
	return &ImageGenerationJobDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *ImageGenerationJobClient) DeleteOne(_m *ImageGenerationJob) *ImageGenerationJobDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *ImageGenerationJobClient) DeleteOneID(id int64) *ImageGenerationJobDeleteOne {
	builder := c.Delete().Where(imagegenerationjob.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &ImageGenerationJobDeleteOne{builder}
}

// Query returns a query builder for ImageGenerationJob.
func (c *ImageGenerationJobClient) Query() *ImageGenerationJobQuery {
	return &ImageGenerationJobQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeImageGenerationJob},
		inters: c.Interceptors(),
	}
}

// Get returns a ImageGenerationJob entity by its id.
func (c *ImageGenerationJobClient) Get(ctx context.Context, id int64) (*ImageGenerationJob, error) {
	return c.Query().Where(imagegenerationjob.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *ImageGenerationJobClient) GetX(ctx context.Context, id int64) *ImageGenerationJob {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *ImageGenerationJobClient) Hooks() []Hook {
	return c.hooks.ImageGenerationJob
}

// Interceptors returns the client interceptors.
func (c *ImageGenerationJobClient) Interceptors() []Interceptor {
	return c.inters.ImageGenerationJob
}

func (c *ImageGenerationJobClient) mutate(ctx context.Context, m *ImageGenerationJobMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&ImageGenerationJobCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&ImageGenerationJobUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&ImageGenerationJobUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&ImageGenerationJobDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown ImageGenerationJob mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		...
		Group, IdempotencyRecord, IdentityAdoptionDecision, ImageGenerationJob,
