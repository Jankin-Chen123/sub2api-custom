...
// The ImageGenerationJobFunc type is an adapter to allow the use of ordinary
// function as ImageGenerationJob mutator.
type ImageGenerationJobFunc func(context.Context, *ent.ImageGenerationJobMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ImageGenerationJobFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ImageGenerationJobMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ImageGenerationJobMutation", m)
}
