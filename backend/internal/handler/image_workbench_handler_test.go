...
func TestWorkbenchCreateHTTPHandlerPersistsDedicatedJobAndPayload(t *testing.T) {
	...
	runtime.Start()
	defer runtime.Stop()
	h := NewDedicatedImageHandler(orchestrator, repo, nil, payloads, billing, runtime, openAI, nil, cfg)

	estimateReq := httptest.NewRequest(http.MethodPost, "/api/v1/user/image-workbench/estimate", bytes.NewReader([]byte(`{"api_key_id":22,"model":"gpt-image-2.5-sunburst-1k"}`)))
	...
	require.Equal(t, service.CangyuanImage25SunburstModel1K, estimateResponse["model"])
	require.Equal(t, 0.1, estimateResponse["estimated_cost"])

	body := []byte(`{"api_key_id":22,"model":"gpt-image-2.5-sunburst-1k","prompt":"draw a small orange puppy","size":"1024x1024","quality":"max"}`)
	...
	require.Equal(t, service.CangyuanImage25SunburstModel1K, job.PublicModel)
	...
	require.Equal(t, "max", stored.Request.Quality)
	require.True(t, stored.Request.Async)
	require.Empty(t, stored.Request.AspectRatio)
	require.Empty(t, stored.Request.ImageSize)
	require.Empty(t, stored.Request.OutputResolution)

	var response map[string]any
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &response))
	require.Equal(t, job.JobID, response["id"])
	require.Equal(t, "draw a small orange puppy", response["name"])
	require.Equal(t, "max", response["quality"])
