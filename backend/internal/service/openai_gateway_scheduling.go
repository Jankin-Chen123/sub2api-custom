...
// grokPreviousResponseSessionSeed returns a stable sticky seed from a Grok
// Responses previous_response_id. Only response IDs are accepted.
...
// grokStickyAffinitySeed scopes sticky routing by model so switching models
// does not inherit a stale account binding.
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	...
	for {
		selection, err := s.selectAccountWithLoadAwareness(ctx, groupID, PlatformOpenAI, sessionHash, requestedModel, effectiveExcludedIDs, false, "", true)
		if err != nil || selection == nil || selection.Account == nil || !selection.Account.IsImageOnly() {
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	...
	for i := range accounts {
		...
		if s.isOpenAIAccountBlockedBySchedulingThreshold(ctx, acc) {
			continue
		}
