...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	newcomerCampaignService := service.ProvideNewcomerCampaignService(client, affiliateService, billingCacheService, apiKeyAuthCacheInvalidator)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, newcomerCampaignService)
	userService := service.NewUserService(userRepository, settingRepository, apiKeyAuthCacheInvalidator, billingCache)
	redeemCache := repository.NewRedeemCache(redisClient)
	redeemService := service.ProvideRedeemService(redeemCodeRepository, userRepository, subscriptionService, redeemCache, billingCacheService, client, apiKeyAuthCacheInvalidator, affiliateService, newcomerCampaignService)
	...
	userHandler := handler.ProvideUserHandler(userService, authService, emailService, emailCache, affiliateService, serviceUserPlatformQuotaRepository, newcomerCampaignService)
	...
	checkinService := service.NewCheckinService(checkinRepository, billingCacheService, settingRepository)
	...
	channelMonitorAccountProbe := service.ProvideChannelMonitorAccountProbe(apiKeyRepository, accountRepository, settingRepository, gatewayService, openAIGatewayService, geminiMessagesCompatService)
	channelMonitorService := service.ProvideChannelMonitorService(channelMonitorRepository, secretEncryptor, settingService, channelMonitorAccountProbe)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, newcomerCampaignService)
	imageGenerationJobRepository := repository.NewImageGenerationJobRepository(db)
	imageGenerationPayloadStore := repository.NewDurableImageGenerationPayloadStore(db, redisClient, secretEncryptor)
	resolvingImageGenerationResultStore := service.ProvideImageGenerationResultStore(imageStorageSettingService, configConfig)
	imageGenerationBilling := service.ProvideImageGenerationBilling(usageBillingRepository, apiKeyService, accountRepository, usageLogRepository, billingCacheService)
	imageGenerationAccountSelector := service.ProvideImageGenerationAccountSelector(openAIGatewayService, accountRepository, configConfig)
	imageGenerationProviderFactory := service.ProvideImageGenerationProviderFactory()
	imageGenerationQueueController := service.ProvideImageGenerationQueueController(imageGenerationJobRepository, settingService, concurrencyService)
	imageGenerationWorker := service.ProvideImageGenerationWorker(imageGenerationJobRepository, imageGenerationPayloadStore, resolvingImageGenerationResultStore, imageGenerationBilling, imageGenerationAccountSelector, imageGenerationProviderFactory, imageGenerationQueueController, configConfig)
	imageGenerationWakeup := repository.NewImageGenerationWakeup(redisClient)
	imageGenerationWorkerRuntime := service.ProvideImageGenerationWorkerRuntime(imageGenerationWorker, imageGenerationWakeup, configConfig)
	imageGenerationCleanupService := service.ProvideImageGenerationCleanupService(imageGenerationJobRepository, imageGenerationPayloadStore, resolvingImageGenerationResultStore, resolvingImageGenerationResultStore, configConfig)
	dedicatedImageRuntimeController := service.ProvideDedicatedImageRuntimeController(settingService, imageGenerationWorkerRuntime, imageGenerationCleanupService, configConfig)
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, dedicatedImageRuntimeController)
	...
	newcomerCampaignHandler := admin.NewNewcomerCampaignHandler(newcomerCampaignService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, adminCheckinHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, newcomerCampaignHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, coordinator, codexDedicatedImageBridge)
	...
	dedicatedImageHandler := handler.NewDedicatedImageHandler(imageGenerationOrchestrator, imageGenerationJobRepository, resolvingImageGenerationResultStore, imageGenerationPayloadStore, billingService, imageGenerationWorkerRuntime, openAIGatewayHandler, imageStorageSettingService, configConfig)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, checkinHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, dedicatedImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
